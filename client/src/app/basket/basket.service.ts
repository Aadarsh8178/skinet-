...
import { IBasketItem,IBasket, Basket, IBasketTotals } from '../shared/models/basket';
...
export class BasketService {
  ...
  updateItemQuantity(item: IBasketItem,add : number) {
    const basket = this.getCurrentBasketValue();
    if(basket) {
      const index = basket.items.findIndex(x => x.id === item.id);
      basket.items[index].quantity +=add;
